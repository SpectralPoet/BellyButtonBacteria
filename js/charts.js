...
function buildCharts(sample) {
  ...
  d3.json("./../samples.json").then((data) => {
    ...
    // 6. Create variables that hold the otu_ids, 
    ...
    var yticks = otuIdArray.slice(0,10).map(i => "OTU " + i).reverse()
    var hoverText = otuLabelsArray.slice(0,10).reverse()
    var sampleValuesArrayDesc = sampleValuesArray.slice(0,10).reverse()

    // 8. Create the trace for the bar chart. 
    var barData = [
      {
        x: sampleValuesArrayDesc,
        y: yticks,
        text: hoverText,
        type: "bar",
        orientation: 'h'
    ...
    var metadata = data.metadata;
    var filtered = metadata.filter(sampleObj => sampleObj.id == sample);
    var wfreq = filtered[0].wfreq;
    console.log(wfreq)
